...
export const login = async (req, res) => {
  try {
    ...
    let user = await repos.User.findByEmail(email);
    if (!user) {
      user = await repos.User.findByUsername(username);
    }

    if (!user)
      return res.status(401).json({ message: "Invalid credentials Email or Username" });
...
export const logout = (req, res) => {
  return res
    .status(200)
    .json({ message: "Logged out — delete token client-side" });
};
